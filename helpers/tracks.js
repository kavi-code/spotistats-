...
module.exports = {
  ...
  parseTopTracks: function(data) {
    ...
    for (var i = 0; i < tracks.length; i++) {
      var track = tracks[i];
      var played_at = track.played_at ;
      
      console.log(played_at)
      console.log(Date.parse(played_at))
      console.log(Date.parse(moment().subtract(7, 'days').calendar()))
      console.log(moment().subtract(7, 'days').calendar());
      if(Date.parse(played_at) < Date.parse(moment().subtract(7, 'days').calendar())) {
